...
def tips():
    print("\nAn error occured!")
    time.sleep(0.6)
    print("\n ~ Try using the keywords mention within brackets - () or []")
    time.sleep(0.5)
    print("\n ~ Try restarting it")
    time.sleep(0.5)
...
def view(mycursor, TABLE):
    ...
    if view_db.upper() == 'Y':
        ...
        table = PrettyTable()
        table.field_names = [description[0]
                             for description in mycursor.description]
        for row in result:
            table.add_row(row)
        print(table)
...
# get operation function here--> before frame() -> inside
...
def main(mycursor, mydb, TABLE):
    try:
        ...
        while name_b and author_b and total_b and available_b != "":
            enter_data(name_b, author_b, total_b,
                       available_b, mycursor, mydb, TABLE)
            time.sleep(0.3)
...
def get_Cred(get_key):
    try:
        ...
        print('Selected details :\n  ', 'Host Server :', final_key['HOST'], '\tUser Name :',
              final_key['USER'], '\tDatabase name:', final_key['DATABASE'], '\tTable name:', final_key['TABLE'])
        setup(**final_key)
    except Exception as error:
...
def show_key(keys):
    ...
    try:
        ...
        for values in key_list:
            ...
            print(key_opt[i], ' ~ ', " [", values.strip('}{').replace(
